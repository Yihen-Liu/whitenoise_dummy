...
func (service *NetworkService) ProxyStreamHandler(stream network.Stream) {
	...
	case pb.Reqtype_NewCircuit:
		...
		if clientInfo, ok := service.proxyService.ClientMap[newCircuit.To]; ok {
			...
			err = service.NewSessionToPeer(clientInfo.peerID, newCircuit.SessionId)
			if err != nil {
		...
		for id, _ := range service.discovery.PeerMap {
		...
		err = service.NewSessionToPeer(join, newCircuit.SessionId)
		if err != nil {
